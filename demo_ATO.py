...
from model.model_ATO import ATONet
...
def main():
    ...
    if opt.save_img:
        opt.save_img_dir = '{}/saveImg_ATO/{}_x{}'.format(opt.save_dir, opt.test_dataset, opt.scale)
        if not os.path.exists(opt.save_img_dir):
            os.makedirs(opt.save_img_dir)
    opt.csv_name = '{}/res_ATO_{}_x{}.csv'.format(opt.save_dir, opt.test_dataset, opt.scale)
    ...
    # Build model
    print("===> Building net")
    model = ATONet(opt).to(device)

    # load state dict
    resume_path = join(opt.model_dir, "ATONet_{}x.pth".format(opt.scale))
        ...
        for k, batch in enumerate(test_loader):
            ...
            if not opt.crop:
                ...
                for i in range(lr_m.shape[0]):
                    sr_m[i:i+1] = predict_y(model, lr_m[i:i+1])
                sr_r = predict_y(model, lr_r)
...
def predict_y(model, lr_y):

    sr = []
    for i in range(opt.angular_num * opt.angular_num):
        ref_ind = torch.tensor([i]).to(device)
        lr_y = lr_y.to(device)
        sr_img = model(lr_y, ref_ind)
        sr.append(sr_img)
    sr = torch.cat(sr, 1)  # [N,an2,h,w]
    sr = sr.cpu().numpy()
